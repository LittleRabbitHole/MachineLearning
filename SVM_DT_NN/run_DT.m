...
%% default tree performance on training set
ytrain_pred=predict(new_tree,x);
error_train = sum(abs(ytrain_pred - y))/size(y,1)
...
%% part b
%new2_tree=fitctree(x,y,'MaxNumSplits',25, 'MinParentSize',20,'MinLeafSize',2,'splitcriterion','gdi', 'PruneCriterion', 'impurity');
%new2_tree=fitctree(x,y,'MaxNumSplits',25, 'MinParentSize',20,'MinLeafSize',15,'splitcriterion','gdi', 'PruneCriterion', 'impurity');
new2_tree=fitctree(x,y,'MaxNumSplits',25, 'MinParentSize',20,'MinLeafSize',15,'splitcriterion','deviance', 'PruneCriterion', 'impurity');
%%% show the tree logic
view(new2_tree);
%%% show the graphics of the tree
view(new2_tree,'Mode','graph');
%% default tree performance on training set
ytrain_pred=predict(new2_tree,x);
error_train = sum(abs(ytrain_pred - y))/size(y,1)

%% evaluate the tree on the test data
y_pred=predict(new2_tree,x_test);
error= sum(abs(y_pred-y_test))/size(y_test,1)